...
CONFIG_FILE="${SCRIPT_DIR}/config/local-simple-catalog-publisher.yaml"
SAMPLE_CONFIG_FILE="${REPO_ROOT}/common/sample-configs/local-simple-catalog-publisher-sample.yaml"
...
replace_placeholders() {
  ...
  while IFS= read -r line || [[ -n "$line" ]]; do
    ...
    line=${line//\{\{catalogBaseURL\}\}/$CATALOG_BASE_URL}
    printf '%s\n' "$line"
...
echo "Welcome to Catalog Publisher configuration!"
echo "Enter the BPP/PN configuration details. These will be available in the setup section of the ION Devlabs portal."

read -rp "Subscriber Id: " SUBSCRIBER_ID
read -rp "Private Key: " PRIVATE_KEY
read -rp "Public Key: " PUBLIC_KEY
read -rp "Key ID: " KEY_ID
read -rp "Web accessible path for the catalog (e.g. https://criteria-overheat-modular.ngrok-free.dev/beckn): " CATALOG_BASE_URL

backup_file=""

if [[ -f "$CONFIG_FILE" ]]; then
  echo
  echo "An existing BPP configuration file was found at:"
  echo "  $CONFIG_FILE"
  ...
  case "$CONFIRM_LOWER" in
    yes|y)
      ...
      backup_file="${backup_directory}/local-simple-catalog-publisher.yaml"
...
echo "Use ' docker compose -f docker-compose-CatalogPublisher.yml up --build -d' to run the Catalog Publisher"